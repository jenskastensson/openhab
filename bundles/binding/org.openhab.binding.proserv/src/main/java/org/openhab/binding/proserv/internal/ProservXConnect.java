...
public class ProservXConnect {
	...
	public void handleProservConnectServer() {
		if (!bDoneProservConnectServer) {
			...
			if(diff > 1000*60*60){ // resend ~hour
			//if (diff > 1000 * 60) {
	...
	private boolean doProservConnectServer() {
		...
		try {
			...
			// DS & VPN: OK
			if (MacID.isEmpty()) {
				// try to find real localIp from hostname (this happens VPN over
				// real)
		...
		if (bFoundGIP && bFoundLIP && bFoundMAC) {
			...
			try {
				logger.info("Sending to ProservConnectServer: MacID: {}, publicIp: {}, localIp: {}", MacID, publicIp, localIp);
				bRet = sendToProservConnectServer(MacID, publicIp, localIp);
			} catch (Exception e) {
	...
	private boolean sendToProservConnectServer(String MAC, String PIP, String LIP) throws Exception {
		...
		logger.debug("Response from ProservConnectServer: {}", response.toString());
		if (response.length() > 0 && response.toString().toLowerCase().indexOf("method=set") != -1
